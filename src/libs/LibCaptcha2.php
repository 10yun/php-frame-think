 ...
 * 生成验证码类 
 * 单例模式 
 * 类会自动开启session,并将使用md5加密后写入$_SESSION['captcha'] .
 * @author cy________
...
class LibCaptcha2
{
     ...
     * @return object 类的实例
     */
    public static function getInstance($imgspathyzm = null)
    {
        if (!(self::$captchaInstance instanceof self)) {
            self::$captchaInstance = new self();
        }
        self::$captchaInstance->fontDir = dirname(__DIR__) . '/assets/fonts/';
        return self::$captchaInstance;
    }
    /**
     * 生成验证码图片的方法
     */
    public function startCreateCaptcha($sessionsign = null)
    {
    ...
    /**
     * 生成验证码
     * @param int $randLength 验证码串的长度
     * @return string $rand 生成的验证码串
     */
    private function randNum($randLength = null)
    {
    ...
    /**
     * 创建图像资源
     * @param string $width 图像的宽        	
     * @param string $height 图像的高        	
    ...
    /**
     * 将随机数写入验证码
     * @param object $im 图像资源
     * @param string $r 验证码串
     * @param int $x 验证码宽
     * @param int $y 验证码高
     * @param int $l 验证码长度
     * @param string $d 验证码字体目录
     */
    private function inputRand($im, $r, $x, $y, $l, $d)
    {
        ...
        for ($i = 0; $i < $l; $i++) {
            ...
            // 简化验证码，清晰
            imagettftext($im, $x / $l, 0, $_x, $_y, $color, $d . 'texb1.ttf', $r[$i]);
            $_x += $x / $l;
        }
    }
    ...
    private function __destruct($im)
    {
        unset(self::$randNum);
        imagedestroy($im);
    }
