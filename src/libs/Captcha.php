...
class Captcha
{
     ...
     */
    public function check(string $captcha_code, string $captcha_key = ''): bool
    {
        $cache_key = mb_strtolower("login:captcha_need:value_{$captcha_code}");

        $cacheData = (array)frameCacheGet('CACHE_STORES_REDIS', $cache_key);
        if (empty($cacheData)) {
            return false;
        }
        ...
        //     return false;
        // }
        $code = mb_strtolower($captcha_code, 'UTF-8');
        $res = password_verify($code, $key);

        if ($res) {
            // $this->session->delete('captcha');
            frameCacheDel('CACHE_STORES_REDIS', $cache_key);
            // Cache::delete('captcha:' . $code);
        }
        return $res;
    }

    /**
     * 输出验证码并把验证码的值保存到cache中
     * @access public
    ...
    public function create(string $config = null, bool $api = false)
    {
        ...
        // 绘验证码
        $captcha_code = $generator['value'];
        $text = $this->useZh ? preg_split('/(?<!^)(?!$)/u', $captcha_code) : str_split($captcha_code); // 验证码
        ...
        $data = [
            "key" => $captcha_key,
            "img" => "data:image/jpg/png/gif;base64," . $base64,
            "length" => strlen($content),
            // "code" => $generator['value'],
        ];

        $cache_key = mb_strtolower("login:captcha_need:value_{$captcha_code}");
        // $cache_data = array_merge($data, [
        //     'value' => $captcha_code,
        // ]);
        $cache_data = [
            'key' => $captcha_key,
        ...
        frameCacheSet('CACHE_STORES_REDIS', $cache_key, $cache_data, $this->expire);
